...
json_path = os.path.join(os.path.dirname(__file__), "vulaire.json")
api_url = "http://localhost:3000/api/v1/products"  # Change to your actual backend URL
params = {"userId": "6GUhKzHQ2ecuSucQBNQHn1Xg4fO2"} # Brand Owner ID
...
for idx, product in enumerate(products, 1):
    ...
    print(f"[{idx}/{len(products)}] {product.get('name')}: {response.status_code} {response.text}")
